...
class TermExtractor(EntityExtractor):
    ...
    def parse(self, tex_path: str, tex: str) -> Iterator[Term]:
        ...
        data_frame_dict = {"term": matched_terms, "start": matched_terms_start, "end": matched_terms_end, "context_str": matched_terms_context_str}
        ...
        # Everything after here, you replace with your extraction of terms
		# from plaintext segments.t - DEFAULT_CONTEXT_SIZE : end + DEFAULT_CONTEXT_SIZE]
            yield Term(
			text=term_object["term"],
			start=term_object["start"],  # start position of the term in the TeX
			end=term_object["end"],  # end position
			id_=str(index),  # unique ID for each term (index)
			tex_path=tex_path,  # tex_path (what was passed into the function)
