...
def join_hue_locs_and_entites(data_dir, arxivIds, include_inline=False):
    ...
    for arxivId in arxivIds:
        ...
        hueCsv = os.path.join(data_dir, "22-hue-locations-for-equations", arxivId, "hue_locations.csv")
        entCsv = os.path.join(data_dir, "17-detected-equations", arxivId, "entities.csv")
        # Read csvs:
        ...
        outDir = os.path.join(data_dir, "99-formatting-data", arxivId)
        ...
        imgDirPrefix = os.path.join(data_dir, "06-paper-images", arxivId)
...
def draw_boxes(data_dir, arxivIds):
...
def create_training_json(data_dir, arxivIds, train=True, micro_eval=False):
    ...
    if train:
        outDir = os.path.join(data_dir, "CNNTest", "train")
    else:
        if not micro_eval:
            outDir = os.path.join(data_dir, "CNNTest", "val")
        else:
            outDirs = [os.path.join(data_dir, "CNNTest", "val_" + arxivId) for arxivId in arxivIds]
    ...
    if not micro_eval:
        ...
        for arxivId in arxivIds:
            ...
            inDir = os.path.join(data_dir, "99-formatting-data", arxivId)
...
if __name__ == "__main__":
    # Run with path to the data directory. Inside this directory should be
    # 22-hue-locations-for-equations, 17-detected-equations, and 06-paper-images:
    data_dir = sys.argv[1]
    arxivIds = os.listdir(os.path.join(data_dir, '22-hue-locations-for-equations'))
    ...
    create_training_json(data_dir, train_arxivs, train=True)
    print('Step 6/6 : Create Valid set')
    create_training_json(data_dir, val_arxivs, train=False)#, micro_eval=True)
