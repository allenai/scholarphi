...
from explanations.explanations.bounding_box import (
    cluster_nearby_connected_components, find_connected_components, merge_bbs)
...
def find_all_math_tags(input_latex_file: str) -> List[Span]:
    with open(input_latex_file, "rb") as f:
        ...
        # e.g., '/net/nfs.corp/s2-research/figure-extraction/data/distant-data/arxiv/src/0001/cond-mat0001356/s10677c.tex'
        text = bs4.UnicodeDammit(f.read()).unicode_markup
        spans = [Span(matches.start(0), matches.end(0), text[matches.start(
            0):matches.end(0)]) for matches in re.finditer(r'(\$(.*?)\$)', text)]
        return spans
...
def compute_bounding_boxes(diff_png_path: str, input_png_for_bbox_overlay_path: str, output_dir: str):
    ...
    level, clusters, d = cluster_nearby_connected_components(
